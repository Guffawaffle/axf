...
const resolved = {
    capability: { id: "global.stfc-mod.status" }
};
    ...
    const ctx = {
        typeAdapter: synthTypeAdapter({
            ...
            data: {
                ...
                durationMs: 12,
                data: { repoRoot: "/srv/stfc-mod" }
            },
            meta: { capabilityId: "global.stfc-mod.status", adapterType: "cli" }
        })
    };
    const result = await stfcExecute(resolved, ctx);
    assert.equal(result.ok, true);
    assert.deepEqual(result.data, { repoRoot: "/srv/stfc-mod" });
    ...
    const ctx = {
        typeAdapter: synthTypeAdapter({
            ...
            data: {
                ...
                timestamp: "2026-04-27T00:00:00.000Z",
                durationMs: 2,
                error: { message: "Full Linux mod build parity is not implemented yet." },
                hints: ["Use pure-tests instead."],
                data: { unsupported: true }
            },
            meta: { capabilityId: "global.stfc-mod.build", adapterType: "cli" }
        })
    };
    const result = await stfcExecute(resolved, ctx);
...
test("flags non-STFC-shaped output as a structured error", async () => {
    ...
    const result = await stfcExecute(resolved, ctx);
    assert.equal(result.ok, false);
    assert.match(result.error.message, /did not return a recognizable STFC envelope/);
});
