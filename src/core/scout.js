...
export async function scoutWorkspace({ rootDir, check = false, write = false, env = process.env } = {}) {
    const workspace = await readWorkspaceConfig(rootDir);
    const imports = normalizeImports(workspace.imports ?? workspace.scout?.imports ?? []);
    const results = [];
    const changes = [];
    const issues = [];
    ...
    for (const importSource of imports) {
        ...
        if (!SUPPORTED_IMPORT_KINDS.has(importSource.kind)) {
            issues.push({ severity: "error", message: `unsupported scout import kind '${importSource.kind}'` });
            continue;
        }
    ...
    return {
        ok: issues.every((i) => i.severity !== "error") && !(check && changes.length > 0),
        imports: results,
...
async function readWorkspaceConfig(rootDir) {
    ...
    try {
        ...
        if (error.code === "ENOENT") {
            throw new AxError("scout requires axf.workspace.json with declared imports", 2);
        }
...
async function scoutAxInventory(rootDir, importSource, { env }) {
    ...
    const existingFamily = await readJsonIfExists(familyManifestPath(rootDir, familyName));
    const familyManifest = buildFamilyManifest({ rootDir, importSource, inventory, existingFamily });
    ...
    for (const [commandName, command] of Object.entries(generatedCommands)) {
        const reservedArgs = Object.keys(command.args ?? {}).filter((arg) => RESERVED_ARG_NAMES.has(arg));
    ...
    for (const standalone of standaloneCapabilities) {
        const capabilityPath = capabilityManifestPath(rootDir, familyManifest, standalone.commandName);
...
async function readAxInventory(rootDir, importSource, { env }) {
    ...
    if (result.code !== 0) {
        throw new AxError(`ax inventory scout failed for '${importSource.family}' with exit code ${result.code}: ${result.stderr || result.stdout}`, 1);
    }
    try {
        ...
        throw new AxError(`ax inventory scout for '${importSource.family}' did not return valid JSON: ${error.message}`, 1);
    }
}
...
function buildFamilyManifest({ rootDir, importSource, inventory, existingFamily }) {
    const familyName = importSource.family;
    const executionTarget = importSource.executionTarget ?? existingFamily?.executionTarget ?? {
    ...
    const family = {
        ...
        adapterType: importSource.adapterType ?? existingFamily?.adapterType ?? "cli",
        providerArgStyle: importSource.providerArgStyle ?? existingFamily?.providerArgStyle ?? "powershell-pascal",
        lifecycleState: normalizeLifecycleState(importSource.lifecycleState ?? existingFamily?.lifecycleState ?? "active"),
        owner: importSource.owner ?? existingFamily?.owner ?? `module:${familyName}`,
        outputModes: importSource.outputModes ?? existingFamily?.outputModes ?? ["json"],
        policies: importSource.policies ?? existingFamily?.policies ?? ["require_workspace_binding"],
        executionTarget,
        commands: {}
    };
    ...
    for (const axCommand of inventory.commands) {
        ...
        for (const key of ["outputModes", "defaults", "lifecycleState", "policies", "owner"]) {
...
function resolvePublicArgName(parameterName, existingArgs) {
    ...
    for (const [publicName, spec] of Object.entries(existingArgs ?? {})) {
        if (spec?.providerFlag?.replace(/^-+/, "").toLowerCase() === parameterName.toLowerCase()) {
            return publicName;
        }
    }
    return normalized;
...
function buildArgsSchema(axCommand, args, existingArgsSchema) {
    ...
    const required = (existingArgsSchema?.required ?? []).filter((name) => properties[name]);
...
function jsonSchemaType(parameter) {
    ...
    if (/^(SByte|Byte|Int16|UInt16|Int32|UInt32|Int64|UInt64)$/.test(parameter.type ?? "")) return "integer";
    if (/^(Single|Double|Decimal)$/.test(parameter.type ?? "")) return "number";
    if (/^Boolean$/.test(parameter.type ?? "")) return "boolean";
    return "string";
}

function buildStandaloneCapability({ familyManifest, commandName, command, existingCapability }) {
    ...
    const manifest = {
        ...
        argsSchema: mergeArgsSchema(command.argsSchema, existingCapability?.argsSchema),
        outputModes: existingCapability?.outputModes ?? command.outputModes ?? familyManifest.outputModes ?? ["json"],
        sideEffects: command.sideEffects,
        scope,
        lifecycleState: normalizeLifecycleState(existingCapability?.lifecycleState ?? command.lifecycleState ?? familyManifest.lifecycleState ?? "active"),
        defaults: existingCapability?.defaults ?? command.defaults ?? {},
        policies: existingCapability?.policies ?? command.policies ?? familyManifest.policies ?? [],
        owner: existingCapability?.owner ?? command.owner ?? familyManifest.owner,
        argMap: existingCapability?.argMap ?? computeArgMap(command.args ?? {}, familyManifest)
    };
...
async function collectJsonChange(changes, { rootDir, path: filePath, kind, family, command = null, manifest }) {
...
function familyManifestPath(rootDir, familyName) {
    return path.join(rootDir, "manifests", "families", `${familyName}.family.json`);
}

function capabilityManifestPath(rootDir, familyManifest, commandName) {
    const scope = familyManifest.scope ?? "global";
    const idPrefix = scope === "workspace-local" ? "workspace" : "global";
    return path.join(rootDir, "manifests", "capabilities", `${idPrefix}.${familyManifest.family}.${commandName}.json`);
}
