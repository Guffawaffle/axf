...
export function validateCapabilityManifest(manifest, label) {
  ...
  if (
    ...
    !LIFECYCLE_STATES.has(manifest.lifecycleState)
  ) {
    issues.push({
      severity: "error",
  ...
  if (
    ...
    typeof manifest.executionTarget === "object"
  ) {
    if (
      manifest.adapterType === "internal" &&
      !manifest.executionTarget.handler
    ) {
      issues.push({
        severity: "error",
    ...
    if (manifest.adapterType === "cli") {
      ...
      if (
        manifest.executionTarget.target &&
        typeof manifest.executionTarget.target === "object" &&
        !Array.isArray(manifest.executionTarget.target)
      ) {
        const target = manifest.executionTarget.target;
        for (const key of ["relativeTo", "fallbackRelativeTo"]) {
          if (
            ...
            issues.push({
              severity: "error",
              message: `${label}: cli adapter executionTarget.target.${key} must be 'workspace' or 'framework' when declared`,
            });
          }
        }
      }
...
export function validateToolspaceManifest(manifest, label) {
  ...
  if (
    ...
    (typeof manifest.moduleMounts !== "object" ||
      Array.isArray(manifest.moduleMounts))
  ) {
    issues.push({
      severity: "error",
